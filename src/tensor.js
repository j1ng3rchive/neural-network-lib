...
class Tensor {
	...
	fill(value) {
		if(this.isScalar()) {
			...
			this.subtensors = new Iterator(this.sizes[0]).map(index =>
	...
	getSubSizes(index) {
		return this.sizes.slice(1).map((size, sizeDimension) => Tensor.getSubSize(size, sizeDimension, index));
	}

	forEach(fn) {
		return this.forEachRecursive(fn, new Tensor.Path().setDimension(this.getDimension()), 0, this);
	}
	...
	map(fn) {
		return this.mapRecursive(fn, new Tensor.Path().setDimension(this.getDimension()), 0, this);
	}
	...
	some(fn) {
		return this.someRecursive(fn, new Tensor.Path().setDimension(this.getDimension()), 0, this);
	}
	...
	findPath(fn) {
		let foundPath = new Tensor.Path().setDimension(this.getDimension());
	...
	vectorMultiply(vector) {
		return this.vectorMultiplyRecursive(vector, new Tensor.Path().setDimension(this.getDimension() - 1), 0);
	}
	...
	static getSubSize(size, sizeDimension, index) {
		if(size instanceof Function) {
			return Tensor.activateSizeFnWithIndex(size, sizeDimension, index);
		} else {
			return size;
		}
	}

	static activateSizeFnWithIndex(size, sizeDimension, index) {
		if(index == 0) {
			return size(index);
		} else {
